      ...
      <div className="space-y-5">
        <div className="rounded-xl border bg-card p-5">
          ...
          <div className="space-y-3">
            <div>
              <Label>Nome *</Label>
              <Input required value={f.name} onChange={(e) => setF({ ...f, name: e.target.value })} className="mt-1" />
            </div>
            <div>
              <Label>Descrição</Label>
              <Textarea value={f.description} onChange={(e) => setF({ ...f, description: e.target.value })} rows={5} className="mt-1" />
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
              <div>
                <Label>SKU</Label>
                <Input value={f.sku} onChange={(e) => setF({ ...f, sku: e.target.value })} className="mt-1" />
              </div>
              <div>
        ...
        <div className="rounded-xl border bg-card p-5">
          ...
          <div className="mt-3 flex gap-2">
            <Input value={newImg} onChange={(e) => setNewImg(e.target.value)} placeholder="URL da imagem" />
            <Button type="button" variant="outline" onClick={() => { if (newImg) { setF({ ...f, images: [...f.images, newImg] }); setNewImg(""); } }}>
              <Plus className="h-4 w-4" />
            </Button>
          </div>
          <p className="mt-2 text-xs text-muted-foreground">Cole uma URL de imagem. Em produção, conecte o storage de imagens.</p>
        </div>
      </div>
      ...
      <aside className="space-y-5">
        <div className="rounded-xl border bg-card p-5">
          ...
          <div className="space-y-3">
            <div>
              <Label>Preço (R$) *</Label>
              <Input required type="number" step="0.01" min="0" value={f.price} onChange={(e) => setF({ ...f, price: Number(e.target.value) })} className="mt-1" />
            </div>
            <div>
              <Label>Preço promocional (opcional)</Label>
              <Input type="number" step="0.01" min="0" value={f.promo_price ?? ""} onChange={(e) => setF({ ...f, promo_price: e.target.value ? Number(e.target.value) : null })} className="mt-1" />
            </div>
            <div>
              <Label>Estoque *</Label>
              <Input required type="number" min="0" step="1" value={f.stock} onChange={(e) => setF({ ...f, stock: Math.max(0, Math.floor(Number(e.target.value))) })} className="mt-1" />
            </div>
          </div>
        </div>
        ...
        <Button type="submit" className="w-full" size="lg">Salvar produto</Button>
      </aside>
    </form>
  );
}
